...
#-------------------------------------------------------------
    ...
    def add_food_online(self, tile):
            tile.spawnFood()
            self.listFoods.add(tile)
            packet = Package(FOOD_STATE)
            data = Data()
            data.create_food_state_package()  # Passer le tile comme argument
            packet.addData(data)
            packet.packData()
            
            self.network.send_package(packet)
                ...
                for dataPack in value.interact_pkg.data:
                    if dataPack.type == BOB_CONSOME:
                        for bob in self.listBobs:
                            if bob.id == dataPack.data['id'] and bob.color == dataPack.data['color']:
                                bob.energy += dataPack.data['energy']
                                bob.CurrentTile.reduceEnergy(dataPack.data['energy'])  # Use the reduceEnergy method
                                bob.eat_all = dataPack.data['eat_all']
                    if dataPack.type == BOB_KILL:
                        pred = None
                        prey = None
