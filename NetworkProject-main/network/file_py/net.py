...
import pickle
import socket
from network.file_py.pymsg_type import *
import subprocess
import os
import random
import ipaddress
...
class Network:
    ...
    def __init__(self, fixed_port=None):
        self.process = None
        self.port = 0
        ...
        self.fixed_port = fixed_port if fixed_port is not None else random.randint(3000, 4000)

    def run_c_process(self):
        from GameControl.EventManager import EtatJeu
        etat = EtatJeu.getEtatJeuInstance()
        path = "./NetworkProject-main/network/runner"
        if not os.path.exists(path):
            print("Client not found")
            etat.kill()
            return
        c_file = [path, str(self.s_port), str(self.fixed_port)]  # Pass the fixed port to the process
    ...
    def assign_port(self, pkg):
        ...
        self.clientList[color_map.get(main_client.color)] = main_client
    ...
    @staticmethod
    def getNetworkInstance(fixed_port=None):
        if Network.instance is None:
            Network.instance = Network(fixed_port=fixed_port)
...
class Network:
    ...
    def kill_c_process(self):
        try:
            ...
            print(f"Process  pid <==> {self.process.pid} is dead\n")
        except OSError as e:
    ...
    def accept_socket(self):
        ...
        print("\n-------------------------------------------------------\n")
        print('Py | ===> Connected addr is ==>', addr)
        packet = Package(PYMSG_CONNECT_OK)
        self.send_package(packet)
        print("\n-------------------------------------------------------\n")
        print("Py | ===> status ==> Accepted\n")
    ...
    def receive_package(self, pkg):
        ...
        if pkg.size != 0:
            ...
            pkg.byte = b''
        print(" py | Recu | Type ==>", pkg.type, "Port ==> ", pkg.port, "Size ==>", pkg.size)
        self.c_socket.setblocking(1)
        return size

    def flush_socket(self):
        self.c_socket.recv(1024)
    ...
    def remove_client(self, pkg):
        ...
        for key, value in self.clientList.items():
            if value != None and value.port == pkg.port:
                ...
                self.clientList[key] = None
                print("Py | Player Key==>", key, "removed")
                break
...
class Data:
    ...
    def create_bob_status_package(self, bob):
        ...
        self.data = {'id': bob.id, 'color': bob.color, 'currentTile': bob.CurrentTile.getGameCoord(), 'previousTiles': [tile.getGameCoord() for tile in bob.PreviousTiles], 'energy': bob.energy, 'mass': bob.mass, 'velocity': bob.velocity, 'speed': bob.speed, 'vision': bob.vision, 'memoryPoint': bob.memoryPoint}

    def create_bob_consome(self, bob, energyEaten, eat_all):
        self.type = BOB_CONSOME
        self.data = {'id': bob.id, 'color': bob.color, 'energy': bob.energy, 'energyEaten': energyEaten, 'eat_all': eat_all}

    def create_bob_kill(self, eater, prey):
        self.type = BOB_KILL
        self.data = {'eater_id': eater.id, 'eater_color': eater.color, 'prey_id': prey.id, 'prey_color': prey.color}

    def create_bob_mate(self, bob1, bob2, child):
        self.type = BOB_MATE
        self.data = {'bob1_id': bob1.id, 'bob1_color': bob1.color, 'bob2_id': bob2.id, 'bob2_color': bob2.color, 'child_id': child.id, 'child_color': child.color}
    ...
    def create_food_state_package(self):
        ...
        for row in game.grid:
            for tile in row:
                if tile.getEnergy() != 0:
                    self.data.append({'coord': tile.getGameCoord(), 'foodEnergy': tile.getEnergy()})
...
class Network:
    ...
    def kill_c_process(self):
        try:
            ...
            print(f"erooor killing process {self.process.pid}: {e}")
